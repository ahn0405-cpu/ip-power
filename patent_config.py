...
# ── 초당 호출 수의 천장 ───────────────────────────────────────────────
# KIPRISplus 의 제약은 일일 총량이 아니라 **초당 호출 수**이고, 넘기면 권한이
# 사라진다. 워커 수를 조절하는 것으로는 못 막는다 — 실제 속도는
# '워커 수 ÷ 응답시간' 이고 응답시간은 우리 것이 아니기 때문이다.
# 자세한 사정은 kipris_rate.py 머리글에 적어 두었다.
#
# 계약상 한도는 100회/초다(2026-09-21 사용자 확인). 그런데 75 로 둔다 — 한도까지
# 쓰지 않는 쪽을 택한 것이다. 넘겼을 때의 대가가 '느려짐' 이 아니라 '권한 상실'
# 이라 비대칭이고, 되돌리는 데 사람 손이 든다. 25% 를 여유로 남긴다.
#
# 이 천장은 **프로세스마다** 걸린다. 자동 실행 둘(daily·weekly)은 concurrency
# 그룹 power-news-pages 를 공유해 겹치지 않으므로 합쳐도 75 를 넘지 않는다.
# 수동 프로브(probe-kipris·probe-foreign-fields)는 그룹 밖이니, 수집이 도는
# 중에 손으로 돌리면 그만큼 더해진다는 점만 알아 두면 된다.
#
# 최대 건수(해외 국적 4,000 + 국내 4,000 + CPC 5,000 = 13,000)를 다 써도
# 13,000 ÷ 75 ≈ 3분이라 매일 실행에 얹기 넉넉하다.
KIPRIS_RPS = float(os.getenv("KIPRIS_RPS", "75"))

# ── 언제 그만둘지 (천장과 다른 물음이다) ─────────────────────────────
# 천장은 '얼마나 빨리' 만 정한다. 차단당한 뒤에도 남은 호출을 꼬박 다 두드리는
# 것은 천장으로 막히지 않는다 — 그래서 차단기를 따로 둔다(kipris_rate.Breaker).
#
# 성공 없이 **연속** 이만큼 실패하면 이번 실행의 KIPRIS 호출을 끊는다. 50 인 이유:
# 정상 실행에도 실패는 섞이지만 연속 50회는 드물고, 반대로 상대가 문을 닫으면
# 실패가 예외 없이 줄줄이 온다. 그 둘을 가르는 자리다.
# 0 이면 이 기준을 끈다(한도 초과를 명시하는 응답은 그래도 한 번에 끊는다).
KIPRIS_FAIL_STREAK = int(os.getenv("KIPRIS_FAIL_STREAK", "50"))
...
SUBGROUPS = {
    "y04s10": [
        ...
        {"code": "10/14", "name": "에너지 저장",
         ...
         # 장주기만 세지 않는다. 계통에 붙은 저장(H02J15·H02J3/28·3/32)과
         # 전지 상태 진단(G01R31/36~/39x, SoC·SoH)을 함께 담는다 — 실측 1,413건
         # 중 진단 쪽이 더 많고, 거기에는 전기차·기기용도 섞여 있다.
         "note": "계통에 붙은 저장장치와 전지 상태 진단(SoC·SoH)을 함께 셉니다"
                 " — 장주기 ESS 만 추린 수치가 아닙니다.",
